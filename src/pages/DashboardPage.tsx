...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onNavigate, userName = 'Arjun' }) => {
  ...
  const weather = kundli?.predictions?.today;

  const pillars = [
    { label: 'Career & Ambition', score: weather?.career?.score || 88, icon: Briefcase, headline: weather?.career?.headline || 'High focus', insight: weather?.career?.insight || 'Strategic meetings favored.' },
    { label: 'Love & Harmony', score: weather?.love?.score || 94, icon: Heart, headline: weather?.love?.headline || 'Heart connection', insight: weather?.love?.insight || 'Open vulnerable communication.' },
    { label: 'Wealth & Labha', score: weather?.finance?.score || 82, icon: DollarSign, headline: weather?.finance?.headline || 'Resource stability', insight: weather?.finance?.insight || 'Focus on long-term assets.' },
    { label: 'Health & Vitality', score: weather?.health?.score || 85, icon: Activity, headline: weather?.health?.headline || 'Pranic strength', insight: weather?.health?.insight || 'Hydrate with herbal infusions.' },
    { label: 'Family & Roots', score: weather?.family?.score || 90, icon: Users, headline: weather?.family?.headline || 'Domestic warmth', insight: weather?.family?.insight || 'Harmonious family discussions.' },
    { label: 'Spirituality', score: weather?.spirituality?.score || 96, icon: Feather, headline: weather?.spirituality?.headline || 'Intuitive depth', insight: weather?.spirituality?.insight || 'Ideal for meditation.' },
  ];
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="rounded-3xl border border-cosmic-border bg-gradient-to-r from-cosmic-surface via-cosmic-card to-cosmic-surface p-6 sm:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-cosmic-card relative overflow-hidden">
        <div className="space-y-1.5 relative z-10">
          ...
          <p className="text-xs sm:text-sm text-cosmic-muted max-w-xl">
            Today's sidereal coordinates present strong alignment for execution and grounded spiritual discernment.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 relative z-10">
          ...
          <div className="p-4 rounded-2xl bg-cosmic-surface border border-cosmic-border text-center min-w-[120px]">
            ...
            <span className="text-sm font-extrabold text-cosmic-text mt-1 block">
              {kundli?.moonSign?.signName || 'Taurus'}
            </span>
            <span className="text-[10px] text-cosmic-gold font-medium">
              {kundli?.moonNakshatra?.name || 'Rohini'}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
        <div className="p-4 rounded-2xl border border-cosmic-border bg-cosmic-surface flex items-center justify-between">
          <div>
            ...
            <span className="font-bold text-cosmic-text text-sm">{kundli?.dashas?.currentMahadasha?.planet || 'Jupiter'}</span>
          </div>
        ...
        <div className="p-4 rounded-2xl border border-cosmic-border bg-cosmic-surface flex items-center justify-between">
          <div>
            ...
            <span className="font-bold text-cosmic-text text-sm">{kundli?.dashas?.currentAntardasha?.planet || 'Venus'}</span>
          </div>
        ...
        <div className="p-4 rounded-2xl border border-cosmic-border bg-cosmic-surface flex items-center justify-between">
          <div>
            ...
            <span className="font-bold text-cosmic-text text-sm">{kundli?.ascendant?.details?.signName || 'Libra'}</span>
          </div>
          <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-400 border border-amber-500/30">
            {kundli?.ascendant?.details?.degreeInSign || 14}°
          </span>
        </div>
      </div>

      {/* 6 Core Life Pillars Grid */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-3xl border border-cosmic-border bg-cosmic-surface p-6 space-y-4">
          ...
          <div className="space-y-3 text-xs">
            <div className="p-3 rounded-xl bg-cosmic-card/60 border border-cosmic-border/60 flex items-center justify-between">
              ...
              <span className="font-bold text-emerald-400">{weather?.favorableHours || '10:30 AM – 12:45 PM'}</span>
            </div>
            <div className="p-3 rounded-xl bg-cosmic-card/60 border border-cosmic-border/60 flex items-center justify-between">
              <span className="text-cosmic-muted">Caution (Rahu Kalam):</span>
              <span className="font-bold text-amber-400">{weather?.cautionHours || '04:30 PM – 06:00 PM'}</span>
            </div>
            <div className="p-3 rounded-xl bg-cosmic-card/60 border border-cosmic-border/60 flex items-center justify-between">
              <span className="text-cosmic-muted">Supportive Direction:</span>
              <span className="font-bold text-cyan-400">{weather?.helpfulDirection || 'North-East'}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-cosmic-border bg-cosmic-surface p-6 space-y-4">
          <div className="flex items-center gap-2 text-xs font-bold text-cyan-400 uppercase tracking-wider">
            <Feather className="w-3.5 h-3.5" /> Suggested Vedic Action & Mantra
          </div>
          <p className="text-xs text-cosmic-text leading-relaxed">
            {weather?.suggestedAction || 'Ground your morning focus facing east and align with the harmonic currents of your ascendant.'}
          </p>
          <div className="p-3.5 rounded-xl border border-cyan-500/30 bg-cyan-500/10 text-xs">
            <span className="text-[10px] uppercase font-bold text-cyan-400 block mb-1">Today's Harmonizing Chanting Mantra</span>
            <span className="text-sm font-display font-extrabold text-cosmic-text">
              {weather?.dailyMantra || 'Om Gurave Namaha'}
            </span>
          </div>
        </div>
      </div>
